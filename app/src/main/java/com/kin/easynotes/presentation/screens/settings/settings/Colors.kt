        ...
        LazyColumn {
                ...
                SettingsBox(
                        ...
                        switchEnabled = {
                            settingsViewModel.update(
                                    settingsViewModel.settings.value.copy(minimalisticMode = it)
                            )
                        }
                )
            ...
            item {
                SettingsBox(
                        ...
                        switchEnabled = {
                            settingsViewModel.update(
                                    settingsViewModel.settings.value.copy(automaticTheme = it)
                            )
                        }
                )
            }
            ...
            item {
                ...
                SettingsBox(
                        ...
                        switchEnabled = {
                            settingsViewModel.update(
                                    settingsViewModel.settings.value.copy(amoledTheme = it)
                            )
                        }
                )
            }
            item {
                SettingsBox(
                        ...
                        isEnabled =
                                settingsViewModel.settings.value.amoledTheme &&
                                        settingsViewModel.settings.value.darkTheme,
                        switchEnabled = {
                            settingsViewModel.update(
                                    settingsViewModel.settings.value.copy(extremeAmoledMode = it)
                            )
                        }
                )
                Spacer(modifier = Modifier.height(18.dp))
                ...
                SettingsBox(
                        ...
                        customAction = { onExit ->
                            OnRadiusClicked(settingsViewModel) {
                                settingsViewModel.update(
                                        settingsViewModel.settings.value.copy(cornerRadius = it)
                                )
                                onExit()
                            }
                        }
            ...
            item {
                SettingsBox(
                        settingsViewModel = settingsViewModel,
                        title =
                                if (settingsViewModel.settings.value.viewMode)
                                        stringResource(id = R.string.grid_view)
                                else stringResource(id = R.string.column_view),
                        icon =
                                if (settingsViewModel.settings.value.viewMode)
                        ...
                        switchEnabled = {
                            settingsViewModel.update(
                                    settingsViewModel.settings.value.copy(viewMode = it)
                            )
                        }
                )
            }
            item {
                SettingsBox(
                        settingsViewModel = settingsViewModel,
                        title =
                                if (settingsViewModel.settings.value.sortDescending)
                                        stringResource(id = R.string.sort_descending)
                                else stringResource(id = R.string.sort_ascending),
                        ...
                        switchEnabled = {
                            settingsViewModel.update(
                                    settingsViewModel.settings.value.copy(sortDescending = it)
                            )
                        }
                )
                Spacer(modifier = Modifier.height(18.dp))
            ...
            item {
                SettingsBox(
                        ...
                        description =
                                stringResource(id = R.string.make_search_bar_longer_description),
                        ...
                        switchEnabled = {
                            settingsViewModel.update(
                                    settingsViewModel.settings.value.copy(makeSearchBarLonger = it)
                            )
                        }
                )
            }
                ...
                SettingsBox(
                        ...
                        customAction = { onExit ->
                            OnFontSizeClicked(settingsViewModel) {
                                settingsViewModel.update(
                                        settingsViewModel.settings.value.copy(fontSize = it)
                                )
                                onExit()
                            }
                        }
...
fun OnRadiusClicked(settingsViewModel: SettingsViewModel, onExit: (Int) -> Unit) {
    ...
    var sliderPosition by remember {
        mutableFloatStateOf(((settingsRadius - minimalRadius).toFloat()) / 45)
    }
    val realRadius: Int = (sliderPosition * 45).toInt() + minimalRadius
...
fun OnFontSizeClicked(settingsViewModel: SettingsViewModel, onExit: (Int) -> Unit) {
    ...
    var sliderPosition by remember {
        mutableFloatStateOf(
                ((currentFontSize - minFontSize).toFloat()) / (maxFontSize - minFontSize)
        )
    }
        ...
        Column(
            ...
            Slider(
                    ...
                    onValueChange = { newValue ->
                        ...
                        val newFontSize =
                                minFontSize + ((maxFontSize - minFontSize) * newValue).toInt()
                    }
            )
